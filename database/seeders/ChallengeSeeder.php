...
class ChallengeSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        */
        ...
        $build_a_simple_to_do_list = Challenge::firstOrCreate([
        ...
        $challenge_id = $build_a_simple_to_do_list->id;
        $challenge_slug = $build_a_simple_to_do_list->challenge_slug;

        // select topic/s
        $topic_front_end = Topic::select('id', 'name')->where('name', 'like', '%frontend%')->first();
        $topics = [ $topic_front_end ];
        
        // attach challenge topic/s
        if (count($topics)) {
            foreach ($topics as $topic) {
                $build_a_simple_to_do_list->addTopic($topic);
            }
        }
        ...
        // attach challenge used framework/s
        if (count($frameworks)) {
            foreach ($frameworks as $framework) {
                $build_a_simple_to_do_list->addFramework($framework);
            }
        }
        ...
        // attach challenge used language/s
        if (count($languages)) {
            foreach ($languages as $language) {
                $build_a_simple_to_do_list->addLanguage($language);
            }
        }
        ...
        if (count($packages)) {
            foreach ($packages as $package) {
                $build_a_simple_to_do_list->addPackage($package);
            }
        }
