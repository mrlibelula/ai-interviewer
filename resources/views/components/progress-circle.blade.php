@props(['color' => '#2fd399', 'value' => 0, 'symbol' => '%', 'customCssValue' => null])
<div>
    <div class="flex items-center justify-center w-[8rem] h-[8rem] rounded-full shadow-lg border-[2px] border-gray-200 dark:border-gray-700" 
        style="background: conic-gradient({{ $color }} {{ $customCssValue ? $customCssValue : $value }}%, transparent {{ $value !== 0 ? 3 : $value }}%);"
    >
