<div class="mt-4 flex flex-col gap-y-4 bg-white dark:bg-gray-900 rounded-lg p-6 shadow-md">
    ...
    <div class="flex flex-col lg:flex-row gap-y-4 lg:gap-y-0 items-center gap-x-2">
        ...
        @endisset
        <div class="px-3 py-0.5 text-sm text-center dark:bg-gray-700 rounded-full shadow-md">
            New: <x-bold>{{ ucfirst($is_new ? 'true' : 'false') }}</x-bold>
        </div>
        <div class="px-3 py-0.5 text-sm text-center dark:bg-gray-700 rounded-full shadow-md">
    ...
    @if (count($challenge->tags))
        <div class="-mt-1 flex items-center gap-x-3 text-base text-sky-500">
    ...
    @if (count($challenge->languages))
        <x-h6>Languages</x-h6>
        <div class="flex items-center gap-x-3 text-base text-sky-500">
    ...
    {{-- <code class="border border-gray-400 dark:border-gray-800 bg-white dark:bg-black text-gray-950 dark:text-gray-400 font-mono p-3 rounded-md">
        {{ $challenge->solution_code }}
    </code> --}}
    <x-code :language="$challenge->languages->first()->name ?? ''" :code="$challenge->solution_code" />
